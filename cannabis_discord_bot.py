...
def action_plant(player: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Ready in **{growth_time}s**."
    )


def action_harvest(player: Dict[str, Any]) -> str:
    ready = [p for p in player["plants"] if now_ts() >= int(p["ready_at"])]
    if not ready:
    ...
    for plant in ready:
        ...
        total += max(1, int(base * efficiency))
...
class UpgradeShopView(discord.ui.View):
    ...
    async def _buy(self, interaction: discord.Interaction, item: str) -> None:
        ...
        if item == "farm":
            ...
            await safe_interaction_reply(
                interaction,
                content=f"✅ Farm upgraded to Lv {upgrades['farm_level']} for ${price}.",
            )
            return
