...
class UseItemView(discord.ui.View):
    ...
    async def _use(self, interaction: discord.Interaction, item: str) -> None:
        ...
        msg = None
        if item in {"lamp", "fert", "chem", "salt", "met"} or item in WEAPON_STATS:
            ...
            elif item == "met" and player.get("met", 0) > 0:
                player["met"] -= 1
                player["xp"] += 60
                msg = "🪙 Мёт использован."
    ...
    async def use_met(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await self._use(interaction, "met")
...
async def use_cmd(ctx: commands.Context, item: str) -> None:
    ...
    if item == "salt":
        ...
        return
    if item == "met":
        if player.get("met", 0) <= 0:
            await ctx.send("Нет мёта.")
            return
        player["met"] -= 1
        player["xp"] += 60
        save_data(data)
        await ctx.send("🪙 Мёт использован: +60 XP.")
        return
    if item in WEAPON_STATS:
...
async def raid_cmd(ctx: commands.Context, target: discord.Member) -> None:
    ...
    chance = 0.50
    chance += active_weapon_bonus(attacker)
...
async def help_cmd(ctx: commands.Context) -> None:
    await ctx.send(
        ...
        "!buyfert <n> !buychem <n> !inventory !met [units] !recharge <units> !notify on/off !badges ... "
