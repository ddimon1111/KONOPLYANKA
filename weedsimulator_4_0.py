...
SERVER_URL = os.getenv("WEEDSIM_SERVER_URL", "http://127.0.0.1:5001")
BOT_PREFIX = "!"
...
HTTP_AVAILABLE = True
HTTP_RETRY_AFTER = 0.0
HTTP_BACKOFF_SECONDS = 30
HTTP_RETRIES = 3
...
def load_data() -> Dict[str, Any]:
    if _http_ready():
        try:
            resp = _request_with_retry("get", "/get")
            data = resp.json()
            _http_mark_success()
            return ensure_shape(data)
        except Exception as e:
            _http_mark_failure("GET /get", e)
...
def save_data(data: Dict[str, Any]) -> None:
    ...
    if _http_ready():
        try:
            _request_with_retry("post", "/save", json=data)
            _http_mark_success()
            return
        except Exception as e:
            _http_mark_failure("POST /save", e)
    # Fallback local save
    with DATA_LOCK:
        tmp = f"{DATA_FILE}.tmp"
...
def update_player_http(user_id: str, updates: Dict[str, Any]) -> None:
    ...
    try:
        _request_with_retry(
            "post",
            "/update_player",
            json={"user_id": str(user_id), "updates": updates},
        )
        _http_mark_success()
    except Exception as e:
        _http_mark_failure("POST /update_player", e, actor=str(user_id))


def wait_for_server(max_wait: float = 5.0) -> bool:
    deadline = time.time() + max_wait
    while time.time() < deadline:
        try:
            resp = requests.get(f"{SERVER_URL}/health", timeout=1.5)
            if resp.ok:
                _http_mark_success()
                return True
        except Exception:
...
def _request_with_retry(method: str, path: str, **kwargs: Any) -> requests.Response:
    ...
    for attempt in range(1, HTTP_RETRIES + 1):
        try:
            resp = requests.request(method, f"{SERVER_URL}{path}", timeout=2.0, **kwargs)
            resp.raise_for_status()
            return resp
        except Exception as e:
            last_err = e
            if attempt < HTTP_RETRIES:
                time.sleep(min(2.0, HTTP_RETRY_DELAY * attempt))
...
def _http_ready() -> bool:
    if HTTP_AVAILABLE:
        return True
    return time.time() >= HTTP_RETRY_AFTER


def _http_mark_success() -> None:
    global HTTP_AVAILABLE
    if not HTTP_AVAILABLE:
        log_event("HTTP_INFO", "system", "Local data server connection restored.")
    HTTP_AVAILABLE = True


def _http_mark_failure(endpoint: str, err: Exception, actor: str = "system") -> None:
    global HTTP_AVAILABLE, HTTP_RETRY_AFTER
    was_available = HTTP_AVAILABLE
    HTTP_AVAILABLE = False
    HTTP_RETRY_AFTER = time.time() + HTTP_BACKOFF_SECONDS
    if was_available:
        log_event(
            "HTTP_WARN",
            actor,
            f"{endpoint} failed: {err}. Falling back to local file mode for {HTTP_BACKOFF_SECONDS}s.",
        )
...
async def menu_cmd(ctx: commands.Context) -> None:
    ...
    player = get_player(data, ctx.author)
    save_data(data)
    await ctx.send(tr(player, "menu_title"), view=MainMenuView())
...
class MainMenuView(discord.ui.View):
    ...
    @discord.ui.button(label="💰 Экономика", style=discord.ButtonStyle.primary)
    async def eco(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Экономика: !balance !sell !exchange !daily !leaderboard", ephemeral=True)

    @discord.ui.button(label="🤝 Кооп", style=discord.ButtonStyle.primary)
    async def coop(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Кооп: !farm ... !cartel ... !clan ...", ephemeral=True)

    @discord.ui.button(label="💨 Кайф", style=discord.ButtonStyle.secondary)
    async def high(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Кайф: !smoke !use meth !use salt", ephemeral=True)

    @discord.ui.button(label="⚔️ PvP", style=discord.ButtonStyle.danger)
    async def pvp(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="PvP: !raid @user !duel @user <sum> !betduel ...", ephemeral=True)

    @discord.ui.button(label="🛒 Магазины", style=discord.ButtonStyle.success, row=1)
    async def shop(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Магазины: !upgrade !blackmarket !buyblack !badges shop", ephemeral=True)

    @discord.ui.button(label="🎒 Инвентарь", style=discord.ButtonStyle.success, row=1)
    async def inv(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Инвентарь: !inventory !use <item> !weapon <name>", ephemeral=True)
    ...
    async def settings(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await safe_interaction_reply(interaction, content="Открываю действия...", view=GameMenuView(), ephemeral=True)
...
class BotRunnerThread(QtCore.QThread):
    ...
    def run(self) -> None:
        ...
        async def start_bot() -> None:
            had_error = False
            try:
                if not wait_for_server(max_wait=4.0):
                    self.status_signal.emit("Local server unavailable, fallback to file mode")
...
class BotControlWindow(QtWidgets.QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.tray_btn = QtWidgets.QPushButton("📌 В трей")
        self.min_btn = QtWidgets.QPushButton("🗕 Свернуть")
        ...
        self.open_admin_btn = QtWidgets.QPushButton("🌐 Открыть веб-админку")
        ...
        self.open_admin_btn.clicked.connect(self.open_web_admin)

        for b in [
            self.start_btn, self.stop_btn, self.copy_link_btn, self.min_btn,
            self.tray_btn, self.restart_btn, self.refresh_btn, self.start_server_btn, self.stop_server_btn, self.open_admin_btn
        ]:
            b.setMinimumHeight(34)

        buttons.addWidget(self.start_btn, 0, 0)
        buttons.addWidget(self.stop_btn, 0, 1)
        buttons.addWidget(self.copy_link_btn, 0, 2)
        buttons.addWidget(self.min_btn, 0, 3)
        buttons.addWidget(self.tray_btn, 0, 4)
        buttons.addWidget(self.restart_btn, 1, 0, 1, 2)
        buttons.addWidget(self.refresh_btn, 1, 2)
        buttons.addWidget(self.start_server_btn, 1, 3)
        buttons.addWidget(self.stop_server_btn, 1, 4)
        buttons.addWidget(self.open_admin_btn, 2, 0, 1, 5)
        ...
        self.log_filter.addItems(["ALL", "RAID", "BUY", "USE", "TRANSFER", "ERROR", "WARN"])
        self.log_search = QtWidgets.QLineEdit()
        self.log_search.setPlaceholderText("Фильтр по тексту логов...")
    ...
    def start_local_server(self) -> None:
        if self.server_process and self.server_process.poll() is None:
            ...
            return
        try:
            self.server_process = subprocess.Popen([sys.executable, "server.py", "--nogui"])
            if wait_for_server(max_wait=6.0):
                self.status_label.setText("Сервер запущен ✅")
                log_event("SERVER", "system", "local server started")
            else:
                self.status_label.setText("Сервер стартует (healthcheck не ответил)")
                log_event("SERVER_WARN", "system", "server process started but healthcheck is pending")
        except Exception as e:
    ...
    def open_web_admin(self) -> None:
        QtGui.QDesktopServices.openUrl(QtCore.QUrl(f"{SERVER_URL}/admin"))
        self.status_label.setText("Открыта веб-админка")
    ...
    def refresh_logs(self) -> None:
        ...
        selected_type = self.log_filter.currentText().strip().upper() if hasattr(self, "log_filter") else "ALL"
        text_filter = self.log_search.text().strip().lower() if hasattr(self, "log_search") else ""
        html_lines = []
        for line in lines:
            line_upper = line.upper()
            if selected_type != "ALL" and selected_type not in line_upper:
                continue
